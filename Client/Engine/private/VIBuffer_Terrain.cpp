...
#pragma region VTXMesh
    ...
    vecPoints.resize(m_VertexNum+100);
    ...
    for (int z = 0; z < height; ++z)
    {
        for (int x = 0; x < width; ++x)
        {
            ...
            vecPoints[idx].vTexcoord = XMFLOAT2((float)x / 8192.f, (float)z / 8192.f);
            //m_vertices[idx] = y;
        }
    }
...
#pragma endregion VTXMesh

#pragma region PosNor
    //m_NumVerticesPerRow = width;
    //m_NumVerticesPerCol = height;
