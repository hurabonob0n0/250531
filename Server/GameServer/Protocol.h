...
// ================================================================
//  FPacketWriter / FPacketReader
//
//  방 목록, 탱크 상태처럼 개수가 매번 다른 패킷이 있어서
//  MMORPG 쪽처럼 고정 크기 struct 로는 표현할 수 없다.
//  그래서 직렬화 헬퍼는 남겨두되, ServerCore 의 BufferWriter/BufferReader
//  에서 실제로 쓰던 기능만 남기고 전부 잘라냈다.
//
//  ★ 원본과 달리 범위 검사를 넣었다.
//    예전 operator<< 는 검사 없이 &_buffer[_pos] 에 그냥 썼기 때문에
//    패킷이 버퍼보다 커지면 남의 메모리를 조용히 덮어썼다.
//    여기서는 넘치면 쓰지 않고 m_overflow 를 세운다.
// ================================================================

// ----------------------------------------------------------------
//  탱크 상태 와이어 포맷 상수
//
//  ★ 클라/서버 양쪽이 반드시 같은 값을 써야 한다. 이 파일이 복제본 2벌인 이유가 이것이다.
//
//  예전엔 자세를 4x4 행렬(64B) 통째로 주고받았다. 그 안에서
//  마지막 열 (0,0,0,1) 16B 는 항상 상수였고, 3자유도 회전에 9개를 쓰고 있었다.
//  지금은 위치 3 + 쿼터니언 4 = 28B 다.
// ----------------------------------------------------------------

// 로드휠 개수. 왼쪽 앞->뒤 7개, 그다음 오른쪽 앞->뒤 7개 순서다
// (클라 CTank::Update_Wheels_And_Track 의 fSag 순서와 같아야 한다).
constexpr int TANK_WHEEL_COUNT = 14;

// 서스펜션 상하 이동량의 한계. PhysX 기본값(압축 0.3 + 신장 0.1)이다.
// 이 범위를 int8 127칸에 나눠 담으므로 한 칸이 0.0031 이고,
// 바퀴 반지름 0.25 의 1.3% 라 화면에서 구분되지 않는다.
// 바퀴 14개를 float 로 보내면 56B 인데 이렇게 하면 14B 다.
constexpr float TANK_WHEEL_SAG_RANGE = 0.4f;

// 실수 -> int8. 범위를 반드시 잘라야 한다.
// 안 자르면 정수가 넘쳐서 부호가 뒤집히고, 바퀴가 위아래로 뒤집혀 튄다.
